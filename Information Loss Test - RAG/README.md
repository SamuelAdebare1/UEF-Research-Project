...
The core research question is: **how much factual information does an LLM lose when it can only see
retrieved fragments of a document, rather than the full text?**

The experiment uses a *needle-in-a-haystack* design. Five highly specific facts ("needles") are
...
Full per-question scores, raw answers, retrieved chunk IDs, and correction notes are in
[`Metrics.csv`](Metrics.csv).

Scores marked `score_auto` are from keyword matching. `score` is the final human-reviewed value —
see `correction_note` for every override.
...
| **Biblical Fact Recall (Q16–Q30)** | 93.3% (14/15) | 73.3% (11/15) |
...
| # | Category | Rationale |
|---|----------|-----------|
| Q1–Q5 | **Needle** | Facts injected into the document that do not exist in any training corpus. A correct answer *requires* retrieval — there is no other source. |
...
|---|----------|----------------|----------------|----------|
| Q19 | What did Adam and Eve sew together? | "They made aprons." | Fig leaves | The chunk clearly states "they sewed fig leaves together, and made themselves aprons." Mistral answered with the *purpose* (aprons) rather than the *material* (fig leaves). The retrieved chunk was correct; the model focused on the wrong semantic element. |
| Q21 | How many years did Methuselah live? | "187 years" | 969 years | 187 is Methuselah's age when he *begat Lamech* — a number that appears two sentences before the total. Mistral extracted the first relevant number rather than the total, a classic partial-context read. |
| Q23 | What did God promise Abram at Sichem? | "the land from the river of Egypt to the Euphrates" | "Unto thy seed will I give this land" | **LLM assumption in action.** The retrieved chunk (chunk 23) contains the Sichem promise (Genesis 12:7). But Mistral answered with the much larger territorial covenant from Genesis 15:18 — a passage that was *not* in the retrieved context. The model overrode retrieved evidence with memorised training knowledge, illustrating exactly the concern raised by the professor. |
...
Auto-scoring used keyword matching (all listed keywords must appear in the answer). Eight answers
were overridden after human review:

| Q | Model | Auto | Corrected | Reason |
|---|-------|------|-----------|--------|
| Q10 | Llama | 0 | 1 | Answered "30 days" (correct); keyword required "thirty" |
| Q20 | Llama | 1 | 0 | Answered "665 years" (wrong); "365" appeared in verbose self-correction text → false positive |
| Q27 | Llama | 0 | 1 | Answered "one hundred three score and fifteen years" (= 175, correct); keyword "threescore" didn't match "three score" |
| Q8 | Mistral | 0 | 1 | Answered "exactly forty-five minutes" (correct); keyword required numeral "45" |
| Q10 | Mistral | 0 | 1 | Answered "30" (correct); keyword required "thirty" |
| Q15 | Mistral | 0 | 1 | Answered "eleven sons" (correct); keyword required numeral "11" |
| Q20 | Mistral | 0 | 1 | Answered "365 years" (correct); keyword required archaic phrasing "sixty and five" |
| Q22 | Mistral | 0 | 1 | Answered "taken by God" (semantically correct); keyword "took him" didn't match passive voice |
...
training memory that directly contradict or ignore the retrieved passage. This validates the
professor's concern: correct retrieval does not guarantee the model will *use* what it retrieved.
...
The documentation identifies `all-mpnet-base-v2` as the model that *"provides the best quality"*
...
`all-MiniLM-L6-v2` is 5× faster and is a reasonable swap if inference time becomes a bottleneck,
but quality is the primary concern in this experiment since a retrieval miss means the needle is
...
```bash
pip install pymupdf tiktoken sentence-transformers fastapi uvicorn gpt4all
```
